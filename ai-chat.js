(() => {
  ...
  @media(max-width:560px){.pc-ai-launch{bottom:calc(68px + env(safe-area-inset-bottom));width:calc(100% - 20px);padding:8px 11px;min-height:56px}.pc-ai-launch .hint{max-width:58vw}.pc-ai-panel{left:8px;right:8px;transform:none;width:auto;bottom:calc(68px + env(safe-area-inset-bottom));height:min(68dvh,620px);max-height:calc(100dvh - 118px);border-radius:20px}.pc-ai-head{padding:10px 11px}.pc-ai-head .mode{display:none}.pc-ai-msg{max-width:94%}.pc-ai-messages{padding:11px}.pc-ai-compose{padding-left:9px;padding-right:9px}}
  `;
  const style=document.createElement('style');style.textContent=css;document.head.appendChild(style);
  const launch=document.createElement('div');launch.className='pc-ai-launch';launch.setAttribute('role','button');launch.setAttribute('tabindex','0');launch.innerHTML='<div class="spark">✦</div><div class="txt"><div class="ttl">Chiedi al Portafoglio</div><div class="hint">Sempre disponibile · conversazione salvata</div></div><div class="chev">⌃</div>';
  const panel=document.createElement('div');panel.className='pc-ai-panel';panel.innerHTML=`<div class="pc-ai-head"><div class="spark">✦</div><div class="title">AI Portafoglio</div><div class="mode" id="pcAiMode">demo locale</div><button class="pc-ai-iconbtn" id="pcAiSettings" aria-label="Impostazioni">⚙︎</button><button class="pc-ai-iconbtn" id="pcAiClose" aria-label="Chiudi">×</button></div><div class="pc-ai-settings" id="pcAiSettingsBox"><label>Endpoint backend OpenAI</label><div class="row"><input id="pcAiEndpoint" placeholder="https://tuo-backend.example/api/chat"><button id="pcAiSaveEndpoint">Salva</button></div><div class="pc-ai-small">La chiave OpenAI resta nel backend. Senza endpoint la barra usa calcoli locali sui dati del portafoglio.</div></div><div class="pc-ai-messages" id="pcAiMessages"></div><div class="pc-ai-compose"><textarea class="pc-ai-input" id="pcAiInput" rows="1" placeholder="Chiedi qualcosa sul portafoglio…"></textarea><button class="pc-ai-send" id="pcAiSend">Invia</button></div>`;
  document.body.append(launch,panel);

  const messagesEl=panel.querySelector('#pcAiMessages'),input=panel.querySelector('#pcAiInput'),send=panel.querySelector('#pcAiSend'),modeEl=panel.querySelector('#pcAiMode'),settingsBox=panel.querySelector('#pcAiSettingsBox'),endpointInput=panel.querySelector('#pcAiEndpoint');
  ...
  async function portfolioContext(){
    await ensureData();
    const d=dataCache||{},s=syncCache||{},positions=d.positions||[];
    const invested=Number(d.liveValue??d.verifiedValue??0),cash=Number(s.cashAvailable??0);
    return{asOf:d.asOf||null,invested,cash,total:invested+cash,positions:positions.map(p=>({name:p.name,category:p.cat||'',qty:p.qty,valueEUR:p.valueEUR,cost:p.cost,dayChangePct:p.dayChangePct}))};
  }
  ...
  async function localAnswer(q){
    ...
    const low=q.toLowerCase(),valued=ctx.positions.filter(p=>Number.isFinite(+p.valueEUR)).sort((a,b)=>b.valueEUR-a.valueEUR),biggest=valued[0],top5=valued.slice(0,5).reduce((s,p)=>s+(+p.valueEUR||0),0),pl=valued.reduce((s,p)=>s+(+p.valueEUR||0)-(+p.cost||0),0);
  ...
  async function askBackend(endpoint,question){const portfolio=await portfolioContext();const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:question,history:history.slice(-16),portfolio})});if(!r.ok)throw new Error(`Backend HTTP ${r.status}`);const out=await r.json();if(!out||typeof out.answer!=='string')throw new Error('Risposta backend non valida');return out.answer}
  ...
  launch.addEventListener('click',openPanel);launch.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openPanel()}});panel.querySelector('#pcAiClose').addEventListener('click',closePanel);panel.querySelector('#pcAiSettings').addEventListener('click',()=>settingsBox.classList.toggle('open'));panel.querySelector('#pcAiSaveEndpoint').addEventListener('click',()=>{const ep=endpointInput.value.trim();if(ep)localStorage.setItem(ENDPOINT_KEY,ep);else localStorage.removeItem(ENDPOINT_KEY);refreshMode();settingsBox.classList.remove('open')});send.addEventListener('click',submit);input.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();submit()}});
