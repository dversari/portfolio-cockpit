(() => {
  ...
  function applyCentral(){
    ...
    try{
      ...
      const xeon=Number(sync.xeonMonetary??0);
      const reserve=Number(sync.buyDipReserve??(cash+xeon));
      ...
      if(sync.asOf)setText('updated',new Date(sync.asOf).toLocaleString('it-IT',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}));
  ...
  async function boot(){
    ...
    try{
      const r=await fetch('fineco_sync.json?central='+Date.now(),{cache:'no-store'});if(!r.ok)return;sync=await r.json();applyCentral();
