const CACHE='portfolio-cockpit-v7';
...
async function injectExtras(response){
  try{
    ...
    if(!text.includes('pull-refresh.js')) text=text.replace('</body>','<script src="pull-refresh.js?v=7"></script></body>');
    if(!text.includes('rumor-radar.js')) text=text.replace('</body>','<script src="rumor-radar.js?v=7"></script></body>');
...
self.addEventListener('fetch',e=>{
  ...
  if(u.pathname.endsWith('/portfolio.json')||u.pathname.endsWith('/fineco_sync.json')||u.pathname.endsWith('/trade_ideas.json')||u.pathname.endsWith('/index.html')){
