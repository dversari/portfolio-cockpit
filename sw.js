const CACHE='portfolio-cockpit-v14';
...
async function injectExtras(response){
  try{
    ...
    if(!text.includes('pull-refresh.js')) text=text.replace('</body>','<script src="pull-refresh.js?v=14"></script></body>');
    if(!text.includes('rumor-radar.js')) text=text.replace('</body>','<script src="rumor-radar.js?v=14"></script></body>');
    if(!text.includes('fx-history.js')) text=text.replace('</body>','<script src="fx-history.js?v=14"></script></body>');
    if(!text.includes('analytics-panel.js')) text=text.replace('</body>','<script src="analytics-panel.js?v=14"></script></body>');
    if(!text.includes('ai-chat.js')) text=text.replace('</body>','<script src="ai-chat.js?v=14"></script></body>');
